...
var fm = (function ($) {
	...
	function validateFeedbackForm(event, position) {
		...
		if ((fm_options.name_required === true && $fm_content.find(".feedback_name").val() === "") ||
				((fm_options.email_required === true && $fm_content.find(".feedback_email").val() === "") || 
				(fm_options.email_required === true && emailValid($fm_content.find(".feedback_email").val()) === false)) ||
				...
				($fm_content.find(".feedback_store").val() !== undefined && $fm_content.find(".feedback_store").val() === "") ||
				$fm_content.find(".type_of_feedback").val() === "") {
			return false;
		}

		if (captcha.length === 0) {
			$("#verify_captcha_message").fadeIn(500);
			setTimeout(function(){ 
				$("#verify_captcha_message").fadeOut(500);
			}, 5000);
			return false;
		}
	...
	function appendFeedbackToBody(fm_options) {
        ...
        email_html += '<li>	<label for="feedback_phone">Phone Number</label><input type="text" class="feedback_phone" id="feedback_phone" placeholder="Your Phone Number" maxlength="10"></input> </li>';
		...
		if (fm_options.show_form === true) {
			form_html = '<form class="feedback_me_form">'
				...
				+ 		'<div class="captcha"><div class="clearfix"></div><div id="recaptchaContainer" style="margin-bottom: 10px;"><div id="ofe_reCaptcha" class="grecaptcha"></div></div><div id="verify_captcha_message" style="display: none;"><span>Please verify security check.</span></div></div>'
				+		'<li>	<button style="margin-top: 10px;" type="submit" onclick="fm.stopPropagation(event);fm.sendFeedback(event,\'' + fm_options.position + '\');" class="feedback_submit ' + bootstrap_btn + '">' + fm_options.submit_label + '</button> </li>'
				+	'</ul>'
				+	'</form>';
		}
	...
	function sendFeedback(event, position) {
		...
		dataArray = {
			store_code: $fm_content.find(".feedback_store").val(),
			name: $fm_content.find(".feedback_name").val(),
			comments: $fm_content.find(".feedback_message").val(),
			email: $fm_content.find(".feedback_email").val(),
			type_of_feedback: $fm_content.find(".type_of_feedback").val(),
            phone: $fm_content.find(".feedback_phone").val(),
